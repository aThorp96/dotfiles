...
class Catalog:
    ...
    def bundles(self) -> list[Bundle]:
        return [Bundle(e) for e in self.entries if e.get("schema") == "olm.bundle"]

def __main__():
    if len(sys.argv) == 1:
        print("No index image provided")

    catalog_image = sys.argv[1]
    catalog = Catalog(catalog_image)
    print(json.dumps(list(b.as_dict() for b in catalog.bundles() if b.name.startswith("openshift-pipelines-operator-rh.v5.0.5"))))
